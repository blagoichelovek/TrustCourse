...
describe("Reentrancy Exercise 1", function () {
  ...
  before(async function () {
    ...
    await (
      await this.bank.connect(user4).depositETH({ value: USER4_DEPOSIT })
    ).wait();

    this.attackerInitialEthBalance = await ethers.provider.getBalance(
      attacker.address
    );
    this.bankInitialBalance = await ethers.provider.getBalance(
      this.bank.address
    );
  ...
  it("Exploit", async function () {
    const AttackContract = await ethers.getContractFactory(
      "contracts/reentrancy-1/AttackContract.sol:Attack",
      attacker
    );
    this.Attacker = await AttackContract.deploy(this.bank.address);
    await this.Attacker.attack({value: ethers.utils.parseEther("1")});
  });
