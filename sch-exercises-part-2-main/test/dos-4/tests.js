...
describe("DOS Exercise 4", function () {
  ...
  it("Failure minting tests", async function () {
    await expect(this.nft.connect(user).mint(20)).to.be.revertedWith(
      "wrong _mintAmount"
    );
    await expect(this.nft.connect(user).mint(1)).to.be.revertedWith(
      "not enough ETH"
    );
    await expect(
      this.nft.connect(user).mint(4, { value: MINT_PRICE.mul(4) })
    ).to.be.revertedWith("exceeded MAX_PER_WALLET");
  });

  it("Pause tests", async function () {
    await expect(this.nft.connect(user).pause(true)).to.be.revertedWith(
      "Ownable: caller is not the owner"
    );
    await this.nft.pause(true);
    await expect(
      this.nft.connect(user).mint(1, { value: MINT_PRICE })
  ...
  it("Exploit", async function () {
    ...
    let tx = await this.nft
      .connect(attacker)
      .mint(2, { value: MINT_PRICE.mul(2) });
    let receipt = await tx.wait();

    let mintedId = receipt.events[0].arg.tokenId;
