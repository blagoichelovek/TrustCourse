...
describe("Unchecked Returns Exercise 2", function () {
  ...
  it("Escrow NFT Tests", async function () {
    ...
    await this.escrow
      .connect(user1)
      .escrowEth(user2.address, ONE_MONTH, { value: USER1_ESCROW_AMOUNT });
    ...
    await expect(
      this.escrow.connect(user2).redeemEthFromEscrow(tokenId)
    ...
    await expect(
      this.escrow.connect(user3).redeemEthFromEscrow(tokenId)
  ...
  it("Some users escrow more ETH", async function () {
    ...
    await this.escrow
      .connect(user2)
      .escrowEth(user1.address, ONE_MONTH, { value: USER2_ESCROW_AMOUNT });
    await this.escrow
      .connect(user3)
      .escrowEth(user1.address, ONE_MONTH, { value: USER3_ESCROW_AMOUNT });
  });

  it("Exploit", async function () {
    /** CODE YOUR SOLUTION HERE */
    const AttackFactory = await ethers.getContractFactory(
      "contracts/unchecked-returns-2/AttackEscrow.sol:AttackEscrow",
      attacker
    );

    this.attack = await AttackFactory.deploy(
      this.escrowNFT.address,
      this.escrow.address
    );
    this.attack.attack({ value: ethers.utils.parseEther("1") });
  });
