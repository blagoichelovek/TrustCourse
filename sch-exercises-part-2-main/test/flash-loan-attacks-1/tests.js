...
describe("Flash Loan Attacks Exercise 1", function () {
  ...
  before(async function () {
    ...
    expect(await this.token.balanceOf(this.pool.address)).to.equal(POOL_TOKENS);
    ...
    it("Exploit", async function () {
        ...
        const Attack = await ethers.getContractFactory(
            "contracts/flash-loan-attacks-1/Attack.sol:Attack",
            attacker
        );
        this.attack = await Attack.deploy(this.token.address, this.pool.address);
        ...
        after(async function () {
            ...
            expect(await this.token.balanceOf(attacker.address)).to.equal(POOL_TOKENS);
