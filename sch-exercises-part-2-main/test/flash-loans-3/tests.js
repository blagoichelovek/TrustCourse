...
describe("Flash Loans Exercise 3", function () {
  ...
  it("Uniswap V2 Flash Swap", async function () {
    ...
    const FlashContract = await ethers.getContractFactory(
      "contracts/flash-loans-3/FlashSwap.sol:FlashSwap",
      impersonatedSigner
    );
    const flashSwapContract = await FlashContract.deploy(WETH_USDC_PAIR);

    // TODO: Send USDC to contract for fees
    await usdc
      .connect(impersonatedSigner)
      .transfer(flashSwapContract.address, FEE_AMOUNT);

    // TODO: Execute successfully a Flash Swap of $40,000,000 (USDC)
    await flashSwapContract
      .connect(impersonatedSigner)
      .executeFlashSwap(USDC_ADDRESS, BORROW_AMOUNT);
  });
});
